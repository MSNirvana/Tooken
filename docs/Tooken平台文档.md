...
- [模块一：平台简介](https://www.notion.so/Tooken-34be6e4ce15f80658bd0c282b3a57a6c?pvs=21)
- [模块二：快速上手](https://www.notion.so/Tooken-34be6e4ce15f80658bd0c282b3a57a6c?pvs=21)
- [模块三：模型与路由](https://www.notion.so/Tooken-34be6e4ce15f80658bd0c282b3a57a6c?pvs=21)
- [模块四：支付与结算](https://www.notion.so/Tooken-34be6e4ce15f80658bd0c282b3a57a6c?pvs=21)
- [模块五：Agent 经济层 · AGI × Web4](https://www.notion.so/Tooken-34be6e4ce15f80658bd0c282b3a57a6c?pvs=21)
- [模块六：API 参考](https://www.notion.so/Tooken-34be6e4ce15f80658bd0c282b3a57a6c?pvs=21)
...
```
# 或使用 curl
curl https://api.[平台域名].com/v1/chat/completions \
...
如果你使用 OpenClaw，可以通过以下方式将 [平台名] 设为默认 API 后端：

```
# 一键配置脚本
curl -fsSL https://[平台域名].com/install/openclaw.sh | bash
```
...
```
{
  "apiBaseUrl": "https://api.[平台域名].com/v1",
...
```
# 设置环境变量
export ANTHROPIC_BASE_URL="https://api.[平台域名].com"
...
```
echo 'export ANTHROPIC_BASE_URL="https://api.[平台域名].com"' >> ~/.zshrc
...
requests.post("https://api.[平台域名].com/v1/roi/event",
...
```
curl https://api.[平台域名].com/v1/route \
  -H "Authorization: Bearer sk-your-api-key" \
  -H "Content-Type: application/json" \
  -d '{
